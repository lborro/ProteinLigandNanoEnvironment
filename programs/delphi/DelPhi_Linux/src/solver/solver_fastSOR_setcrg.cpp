...
void CDelphiFastSOR::setCrg()
{
   ...
   for (vector< SGrid<delphi_integer> >::iterator it = gchrgp.begin(); it != gchrgp.end(); ++it)
   {
      if (0 != optSum<delphi_integer>(*it)%2) { gchrg2.push_back(i); i++; }
   ...
   for (vector< SGrid<delphi_integer> >::iterator it = gchrgp.begin(); it != gchrgp.end(); ++it)
   {
      ...
      if (1 == ibgp || 1 == iext)
      {
         ...
         fdbGridVal.fgCoord.nX = (delphi_real)ix; fdbGridVal.fgCoord.nY = (delphi_real)iy; fdbGridVal.fgCoord.nZ = (delphi_real)iz;
