...
*/
                    ...
                    if(iSurfCrgFormatOut==1 || iSurfCrgFormatOut==2){
						...
						icode = prgapAtomPdb[jj-1].getAtInf()[15];
						bgp = i+1;	
						if (bgp > 99999)
							bgp = 99999;
						ofScrgFile << "ATOM  " << setw(5) << fixed << right << bgp << " " <<  setw(5) << fixed << right << jj << " SC " << setw(6) << fixed << right << qq << icode << "     " << setw(5) << fixed << right << prgfgSurfCrgA[i].nX << "  " << setw(5) << fixed << right << prgfgSurfCrgA[i].nY << "  " << setw(5) << fixed << right << prgfgSurfCrgA[i].nZ << "  " << setw(5) << fixed << right << scientific << fEnergy_Temp1 << "  " << setw(5) << fixed << right << scientific << spt1 << endl;
					}
			}
		}
