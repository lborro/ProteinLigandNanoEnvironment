...
void CSite::writeSite(const int& iisitsf)
{
   ...
   string atm,res,rnum,chn,crdstr,atnum,atdes(17,' '),strLine,strHead;
   char ricode;
   bool isitmp;
   int iFound,iresnum,idist,ncrg,jtmp;
   vector<bool> atmsf(iAtomNum*iisitsf,false);
   vector<delphi_real> sold,scomp;
   char otemp[11]; //must include the null character 10 + \0
   string oline(80,' ');
   ...
   else
   {
      do // beginning of the big loop on natom
      {
	    ...
	    ricode = vctapAtomPdb[nnatom].getAtInf()[15];
         ...
         else
         {
            ...
            if(ifrm2) // formatted reading
            {
               ...
               if (0 != strHead.compare("ATOM  ") && 0 != strHead.compare("HETATM")) continue;
               crdstr = strLine.substr(32,32);
               atnum = strLine.substr(6,5);
               xo.nX = atof(crdstr.substr(0,11).c_str()); xo.nY = atof(crdstr.substr(11,11).c_str()); xo.nZ = atof(crdstr.substr(22,11).c_str());
         ...
         {
            atm  = strLine.substr(12,5);
            res  = strLine.substr(17,3);
            rnum = strLine.substr(21,6);	
	    ...
	    atdes[15] = ricode;
         ...
         if (isita)
         {
            oline.replace(j,17,atdes.substr(0,17));
            j += 20;
         }

         if (isitx)
         {
            snprintf(otemp,11,"%10.4f",xo.nX); oline.replace(j,10,otemp); j += 10;
            snprintf(otemp,11,"%10.4f",xo.nY); oline.replace(j,10,otemp); j += 10;
            snprintf(otemp,11,"%10.4f",xo.nZ); oline.replace(j,10,otemp); j += 10;
         }

         if (isitq)
         {
            snprintf(otemp,11,"%10.4f",chrgv); oline.replace(j,10,otemp); j += 10;
         }

         if (isitp)
         {
            snprintf(otemp,11,"%10.4f",phiv);  oline.replace(j,10,otemp); j += 10;
         ...
         if (isiti)
         {
            snprintf(otemp,11,"%10.4f",phii);  oline.replace(j,10,otemp); j += 10;
         }

         if (isitr)
         {
            snprintf(otemp,11,"%10.4f",phir);  oline.replace(j,10,otemp); j += 10;
         }

         if (isitc)
         {
            snprintf(otemp,11,"%10.4f",phic);  oline.replace(j,10,otemp); j += 10;
         }

         if (isitap)
         {
            snprintf(otemp,11,"%10.4f",aphi);  oline.replace(j,10,otemp); j += 10;
         }

         if (isitdeb)
         {
            snprintf(otemp,11,"%10.4f",debyefraction); oline.replace(j,10,otemp); j += 10;
         }

         if (isitf)
         {
            snprintf(otemp,11,"%10.4f",fxyz.nX); oline.replace(j,10,otemp); j += 10;
            snprintf(otemp,11,"%10.4f",fxyz.nY); oline.replace(j,10,otemp); j += 10;
            snprintf(otemp,11,"%10.4f",fxyz.nZ); oline.replace(j,10,otemp); j += 10;
         }

         if (isitrf)
         {
            snprintf(otemp,11,"%10.4f",rxyz.nX); oline.replace(j,10,otemp); j += 10;
            snprintf(otemp,11,"%10.4f",rxyz.nY); oline.replace(j,10,otemp); j += 10;
            snprintf(otemp,11,"%10.4f",rxyz.nZ); oline.replace(j,10,otemp); j += 10;
         }

         if (isitcf)
         {
            snprintf(otemp,11,"%10.4f",cxyz.nX); oline.replace(j,10,otemp); j += 10;
            snprintf(otemp,11,"%10.4f",cxyz.nY); oline.replace(j,10,otemp); j += 10;
            snprintf(otemp,11,"%10.4f",cxyz.nZ); oline.replace(j,10,otemp); j += 10;
         }

         if (isittf)
         {
            vtemp = rxyz + cxyz;
            snprintf(otemp,11,"%10.4f",vtemp.nX); oline.replace(j,10,otemp); j += 10;
            snprintf(otemp,11,"%10.4f",vtemp.nY); oline.replace(j,10,otemp); j += 10;
            snprintf(otemp,11,"%10.4f",vtemp.nZ); oline.replace(j,10,otemp); j += 10;
         }
         ...
         if (isitt)
         {
            snprintf(otemp,11,"%10.4f",phit); oline.replace(j,10,otemp); j += 10;
         }
      ...
      for (delphi_integer jj = 0; jj < iBndyGridNum; jj++)
      {
         ...
         if (atmsf[i-1] && (0 < prgiAtNdx[jj]))
         {
            ...
            if (ofrm)
            {
               ...
               snprintf(otemp,11,"%10.4f",prgfSurfCrgE[jj]); oline.replace(jtmp,10,otemp); jtmp += 10;
               snprintf(otemp,11,"%10.4f",xo.nX);            oline.replace(jtmp,10,otemp); jtmp += 10;
               snprintf(otemp,11,"%10.4f",xo.nY);            oline.replace(jtmp,10,otemp); jtmp += 10;
               snprintf(otemp,11,"%10.4f",xo.nZ);            oline.replace(jtmp,10,otemp); jtmp += 10;
               snprintf(otemp,11,"%10.4f",fn);               oline.replace(jtmp,10,otemp); jtmp += 10;
               snprintf(otemp,11,"%10.4f",ff);               oline.replace(jtmp,10,otemp); jtmp += 10;
